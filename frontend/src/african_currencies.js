 ...
 * Matches the backend data in webapp/african_currencies.py
 */
...
// Convenience lookups
export const COUNTRY_TO_CURRENCY = Object.fromEntries(
  AFRICAN_COUNTRY_CURRENCIES.map(c => [c[0], c[2]])
);
export const VALID_COUNTRIES = new Set(
  AFRICAN_COUNTRY_CURRENCIES.map(c => c[0])
);
export const VALID_CURRENCY_CODES = new Set(
  AFRICAN_COUNTRY_CURRENCIES.map(c => c[2])
);

export function defaultCurrencyForCountry(country) {
  return COUNTRY_TO_CURRENCY[country] || "TZS";
}
